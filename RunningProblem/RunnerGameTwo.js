class Runner {
    ...
    getMoveCoords(dist) {
        ...
        for (var i = cur_pos.x - dist; i < cur_pos.x + dist; i += 1) {
            for (var j = cur_pos.y - dist; j < cur_pos.y + dist; j += 1) {
                if (sqr(i - cur_pos.x) + sqr(j - cur_pos.y) - sqr(dist) <
                    ...
                    sum += geometryUtils.distanceBetweenToPoints(firstCatcherGameOTwo.getCoords(), {
                        x: i,
                        y: j
                    });
                }
        ...
        for (var i = 1; i < count_coords; i++) {
            if (geometryUtils.distanceBetweenToPoints(firstCatcherGameOTwo.getCoords(), moves[i
            ]) > ave_dist) {
